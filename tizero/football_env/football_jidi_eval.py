...
class FootballJiDiEnv(MultiAgentEnv):
    ...
    def __init__(self,scenario_name:str, cfg:Dict={}, rank:int =0, log_dir=None, isEval=True,seed:int =0,write_video=False,need_render=False):
        ...
        if self.fake_step:
            self.fake_raw,self.fake_r,self.fake_d,self.fake_info = None, None, None, None
    ...
    def reset(self):
        ...
        self.cur_step = 0
        return self.add_control_index(raw_obs), [{} for i in range(self.left_agent_num + self.right_agent_num)]
    ...
    def step(self, actions):
        ...
        if self.fake_step:
            if self.fake_raw is None:
                ...
                self.fake_raw,self.fake_r,self.fake_d,self.fake_info = raw_o, r, d, info
        ...
        for i in range(self.left_agent_num + self.right_agent_num):
            if hasattr(self,"max_episode_length") and self.cur_step>=self.max_episode_length:
                dones.append(True)
            else:
                dones.append(d)
